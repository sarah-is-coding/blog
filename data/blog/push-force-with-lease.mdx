...
<Mermaid chart={`gitGraph
   ...
   commit id: "more history"
   branch my-feature
   commit id: "your commit"
   checkout main
   commit id: "teammate pushes"
   checkout my-feature
   commit id: "your rebase"
`} />

_Each node is a commit — a saved snapshot of the code at a point in time. After you rebase, your local `my-feature` and the remote `my-feature` have gone in different directions: the remote has `teammate pushes`, yours has `your rebase`. A `--force` push overwrites the remote with your version, silently erasing your teammate's commit._
